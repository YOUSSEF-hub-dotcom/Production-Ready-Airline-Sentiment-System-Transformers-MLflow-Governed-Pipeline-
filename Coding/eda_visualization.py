...
def run_eda_visualization(df):
    ...
    # --- 1. Temporal Feature Preparation ---
    # Convert tweet_created column from object/string to datetime object to enable time-based analysis
    logger.info("Convert tweet_created column to datetime")
    df['tweet_created'] = pd.to_datetime(df['tweet_created'])
    print(df.dtypes)

    # --- 2. Global Sentiment Distribution Analysis ---
    # Analyze the overall frequency of Positive, Neutral, and Negative classes to detect class imbalance
    logger.info("Distribution of Sentiments in the Dataset:")
    sentiment_counts = df['airline_sentiment'].value_counts()
    logger.info(f"Sentiment Distribution:\n{sentiment_counts}")

    # Count the volume of tweets received by each airline to understand corporate representation
    logger.info("Number of Tweets per Airline:")
    print(df['airline'].value_counts())

    # Set plot styling and color palette using seaborn's standard specifications
    plt.style.use('seaborn-v0_8')
    sns.set_palette('coolwarm')

    # Plot a simple count bar chart for overall sentiment distribution
    plt.figure(figsize=(6, 4))
    sns.countplot(x='airline_sentiment', data=df)
    plt.title('Distribution of Sentiments')
    plt.show()

    # Calculate and print the percentage distribution of each sentiment category
    ...
    # Generate a Pie Chart to visually represent the massive share of negative feedback
    ...
    # --- 3. Airline-Specific Volume & Sentiment Analysis ---
    # Extract and log the absolute most mentioned airline in the entire dataset
    ...
    # Bar chart showing the distribution of total mentions across all 6 airlines
    ...
    # Cross-tabulation to inspect the exact count breakdown of sentiments for each individual airline
    ...
    # Clustered bar chart displaying the sentiment split (Pos/Neu/Neg) per airline for easy benchmarking
    ...
    # --- 4. Deep-Dive into Negative Complaints ---
    # Filter for negative tweets and identify the top recurring reasons/drivers for customer dissatisfaction
    ...
    # Bar chart of the Top 5 negative complaint categories (e.g., Customer Service, Late Flights)
    plt.figure(figsize=(8, 4))
    ...
    # Determine which airline accumulates the highest total volume of negative complaints
    ...
    # Bar chart comparing absolute volume of negative tweets among all airlines
    plt.figure(figsize=(8, 4))
    ...
    # --- 5. Deep-Dive into Positive Sentiment ---
    # Determine which airline accumulates the highest total volume of positive/praising tweets
    ...
    # Bar chart comparing absolute volume of positive customer feedback across airlines
    plt.figure(figsize=(8, 4))
    ...
    # --- 6. Behavioral & Temporal Pattern Analysis ---
    # Extract the hour from the timestamp to observe peak customer posting windows throughout the day
    ...
    # Count plot showing tweet frequency changes by the hour
    plt.figure(figsize=(8, 4))
    sns.countplot(x='hour', data=df)
    plt.title('Tweets by Hour of Day')
    plt.show()

    # Calculate text metadata length (word count) based on the cleaned text column
    ...
    # Histogram with KDE curve to observe the overall distribution shape of text word counts
    plt.figure(figsize=(8, 4))
    sns.histplot(df['tweet_length'], bins=30, kde=True)
    plt.title('Tweet Length Distribution')
    plt.show()

    # Core Hypothesis Testing: Group by sentiment to verify if frustrated customers write longer texts
    ...
    # Boxplot showing text length variations and outliers across sentiment classes
    sns.boxplot(x='airline_sentiment', y='tweet_length', data=df)
    plt.title('Tweet Length by Sentiment')
    plt.show()

    # Public Engagement Analysis: Check if negative tweets go viral more often via retweet counts
    ...
    # Boxplot comparing distribution of retweets across sentiment categories
    sns.boxplot(x='airline_sentiment', y='retweet_count', data=df)
    plt.title('Retweet Count by Sentiment')
    plt.show()

    # --- 7. User & Location Demographics ---
    # Find the most vocal or frequent individual accounts posting feedback
    ...
    # Bar chart plotting the top 5 most active Twitter usernames
    sns.barplot(x=top_users.index, y=top_users.values)
    plt.title('Top Active Users')
    plt.xticks(rotation=45)
    plt.show()

    # Analyze geographical distribution using the user-provided location field
    ...
    # Bar chart plotting the top 5 geographic metadata locations
    sns.barplot(x=top_locations.index, y=top_locations.values)
    plt.title('Top Tweet Locations')
    plt.xticks(rotation=45)
    plt.show()

    # --- 8. Dataset Labels Confidence Quality Control ---
    # Investigate the mean confidence scores attached to the ground-truth human annotations
    ...
    # Boxplot tracking distribution profile of annotator confidence across classes
    sns.boxplot(x='airline_sentiment', y='airline_sentiment_confidence', data=df)
    plt.title('Sentiment Confidence by Type')
    plt.show()

    # Histogram capturing how confident annotators were when assigning the specific reasons for complaints
    ...
    # --- 9. Advanced Grouped Cross-Airlines Benchmarking ---
    # Determine the absolute number one leading category of complaint per airline
    ...
    # Boxplot investigating if certain airlines attract more viral/shared retweets than others
    ...
    # Matrix aggregation to directly contrast purely Positive vs purely Negative feedback volume per carrier
    ...
    # Grouped/Side-by-side bar chart showing Positive vs Negative metrics comparison per airline
